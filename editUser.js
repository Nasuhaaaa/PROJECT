...
const updateUserDetails = async (staff_ID, staff_name, staff_email, role_ID, department_ID) => {
  return new Promise((resolve, reject) => {
    const query = `
      UPDATE User 
      SET staff_name = ?, staff_email = ?, role_ID = ?, department_ID = ?
      WHERE staff_ID = ?
    `;
    db.query(query, [staff_name, staff_email, role_ID, department_ID, staff_ID], (err, results) => {
      if (err) {
